<template>
    <view class="map-bg px-20 h-200 flex flex-col justify-around text-2xl">
        <view class="flex flex-col h-80 justify-between">
            <view class="w-300 flex p-20 my-10 bg-white text-gray-500 rounded-2xl items-center justify-around shadow"
                  @tap="choosePerson">
            ...
            <view class="w-200 flex p-20 my-10 bg-white text-gray-500 rounded-2xl items-center justify-around shadow"
                  @tap="chooseDate">
    ...
    private choosePerson() {
        ...
        my.complexChoose({
            ...
            success: (res) => {
                ...
                this.users = res.users.length > 0 ? res.users : [{userId: 'NB1466'}]
                // 1. 更新users
